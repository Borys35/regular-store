    ...
    <nav className="flex gap-8">
      {categories.map(({ id, name, slug, children }: any) => (
        <span key={id} className="group relative font-bold">
          ...
          {!!children.length && (
            <ul className="absolute rounded z-10 flex flex-col gap-1 top-full translate-y-1 -left-3 px-3 py-2 border-1 border-accent shadow-md w-max scale-0 origin-top-left transition-transform group-hover:scale-100 bg-white">
